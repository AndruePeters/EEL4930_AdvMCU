...
level_t Levels[NUMLEVELS] = { { 3, 0, 2048, FIX / 4, 1, LCD_GREEN, 60, 0 }, // 0: rolling hills
        ...
        { 20, 4096, 8192, FIX * 2, 15, LCD_WHITE, 1, 0 } };
...
uint32_t FindFreeRocket(void)
{
    ...
    while (Things[i].life)
    {
        i++;
        if (i > ROCKETMAX)
            return 0; // no more player rockets can be created
    }
    return i;
}
uint32_t FindFreeEnemy(void)
{
    ...
    while (Things[i].life)
    {
        i++;
        if (i > ENEMYMAX)
            return 0; // no more enemies can be created
    }
    return i;
}
uint32_t FindFreeEMissile(void)
{
    ...
    while (Things[i].life)
    {
        i++;
        if (i > EMISSILEMAX)
            return 0; // no more enemy missiles can be created
    }
    return i;
}
...
void CreateLand(uint32_t duration, uint32_t max, int32_t city)
{
    ...
    {
        t = ((i - (MAXTERRAIN - 32))
                * (Landscape[0] - Landscape[MAXTERRAIN - 33])) / 32
...
uint32_t Distance(uint32_t n1, uint32_t n2)
...
void ReDrawLand(uint16_t color, uint16_t bgcolor)
{
    ...
    {
        BSP_LCD_DrawFastVLine(i, SCREENHEIGHT - Landscape[i], Landscape[i],
                              color);
...
void DrawSprites(void)
{
    ...
    for (i = 0; i < NUMSPRITES; i++)
    {
        if (Things[i].life)
        {
            BSP_LCD_DrawBitmap(Things[i].x, Things[i].y,
                               Things[i].ImagePt[Things[i].AnimationIndex],
                               Things[i].w, Things[i].h);
...
void MissileHitsShip(void)
{ // check for enemy missiles hitting player ship
    uint32_t i, d;
    for (i = EMISSILEMIN; i <= EMISSILEMAX; i++)
    {
        if (Things[i].life)
        {
            ...
            {
                if (Things[SHIP].life)
                    Things[SHIP].life--; // lose one life
                if (Score > 10)
                {
                    Score = Score - 10;
                }
                ...
                Things[i].life = 0;
                BSP_LCD_DrawBitmap(Things[i].x, Things[i].y,
                                   Things[i].DeathImagePt, Things[i].w,
                                   Things[i].h);
            }
        }
    }
...
/*
 * Edited by Drue
 * If you touch my ghost then you instantly die
 * This is done by checking the y dimmension
 */
void EnemyHitsShip(void)
{ // check for enemies hitting player ship
    uint32_t i, d;
    for (i = ENEMYMIN; i <= ENEMYMAX; i++)
    {
        if (Things[i].life)
        {
            ...
            if (d <= ENEMY2SHIP)
            {
                /* If you're peaceful and dont' shoot */
                if ((CurrentLevel == 0) && (KillsThisLevel == 0))
                {
                    Score = Score + 100;  // good thing
                }
                /* if you were evil and killed them */
                else
                {
                    /*
                     * Only my ghosts have height of 24
                     * So, checking the height is a good way to tell if it's a generic alien or my ghost
                     */
                    if (Things[i].h == 24)
                    {
                        Things[SHIP].life = 0;
...
void ShipHitsLand(void)
{ // check for ship hitting land
    uint32_t t;
    int i;
    t = 0;
...
void RocketHitsMissile(void)
{ // check for rockets hitting missiles
    uint32_t i, j, d;
                ...
                if (Things[j].life)
                {
                    ...
                    if (d <= MISSILE2ROCKET)
                    {
                        ...
                        BSP_LCD_DrawBitmap(Things[i].x, Things[i].y,
                                           Things[i].DeathImagePt, Things[i].w,
                                           Things[i].h);
                        BSP_LCD_DrawBitmap(Things[j].x, Things[j].y,
                                           Things[j].DeathImagePt, Things[j].w,
                                           Things[j].h);
                    }
                }
            }
...
/* Edited by Drue
 * This is mostly the same code as the original, but I added the capability of custom scoring
 * My Pacman ghosts have different dimensions than the rest of the rockets, so I just
 * check the dimension of the object if it was destroyed and if it's 24 then I know it was my ghost
 *
 * Future revisions would simply add an id number to the sprite struct
 */
void RocketHitsEnemy(void)
{ // check for rockets hitting enemies
    uint32_t i, j, d;
    for (i = ROCKETMIN; i <= ROCKETMAX; i++)
    {
        if (Things[i].life)
                ...
                if (Things[j].life)
                {
                    ...
                    if (d <= ROCKET2ENEMY)
                    {
                        ...
                        if (Things[j].life == 0)
                        {
                            if (Things[j].h == 24)
                            {
                                Score += Levels[CurrentLevel].EnemyLife * 4;
                            }
                            else
                            {
                            Score += Levels[CurrentLevel].EnemyLife * 2;
                            }
                            KillsThisLevel = KillsThisLevel + 1;
                            Sound_Killed();
                            Things[j].life = 0;
                            BSP_LCD_DrawBitmap(Things[j].x, Things[j].y,
                                               Things[j].DeathImagePt,
                                               Things[j].w, Things[j].h);
                        }
                        BSP_LCD_DrawBitmap(Things[i].x, Things[i].y,
                                           Things[i].DeathImagePt, Things[i].w,
                                           Things[i].h);
...
void CreateSprite(int i, const unsigned short *livePt,
                  const unsigned short *livePt2, uint32_t animationDuration,
                  const unsigned short *deadPt, short initx, short inity,
                  short width, unsigned height, short initvx, short initvy,
                  int alive)
{
    Semaphore_pend(MutexHandle, BIOS_WAIT_FOREVER);
...
// Output: none
void revealchar(uint16_t x, uint16_t y, char *pt, int16_t textColor,
                int16_t falseColor, uint16_t revealChance)
{
    ...
    else
    {
        ...
        else
        {
            ...
            BSP_LCD_DrawChar(x * 6, y * 10, ('0' + (Random16() % 75)),
                             falseColor, LCD_BLACK, 1);
        }
    }
}
#define NUMRANDCOLORS 12                  // length of random color array
const uint16_t RandomColors[NUMRANDCOLORS] = { 0x801F,  // 255, 0, 128
        0x001F,  // 255, 0, 0
        0x041F,  // 255, 128, 0
        0x07FF,  // 255, 255, 0
        0x07F0,  // 128, 255, 0
...
uint32_t revealstring(uint16_t y, char *pt, int16_t textColor)
{
    ...
    while (*pt)
    {
        revealchar(x, y, pt, textColor, RandomColors[Random() % NUMRANDCOLORS],
                   6554);
        pt++;
        x = x + 1;
        if (x > 25)
...
void Intermission(uint32_t delay10ms, int32_t level)
{
    ...
    for (i = 0; i < 12; i = i + 1)
    {
        ...
        {
            revealstring(2, "The time has come", LCD_WHITE);
            revealstring(4, "For our rhymes to end", LCD_WHITE);
            revealstring(6, "You have only killed some", LCD_WHITE);
            revealstring(8, "Now your end begins", LCD_WHITE);
            revealstring(10, "Good luck", LCD_WHITE);
        }
        else if (level == 5)
        {
        ...
        {
            revealstring(5, "Afraid of ghosts?", LCD_WHITE);
        }
...
{ // runs at about 30 Hz
    uint16_t x, y;
    uint8_t button;
    CreateSprite(SHIP, pac_open_mouth, pac_closed_mouth, 5, pac_blank, 0,
                 DesiredPlace, 18, 18, 0, 0, 10);
    Intermission(300, -1);
    while (Things[SHIP].life)
    {
        ...
        if (IntermissionFlag)
        {
            ...
            if (x > 650)
            {
                if ((Things[SHIP].x < (SCREENWIDTH - 1))
                        && (Things[SHIP].y > Landscape[Things[SHIP].x]))
                {
...
void AddEnemyTask(void)
{
    uint32_t me;
    uint32_t t, j, initY, ok, trys;
    uint32_t random_gen_pac_ghost = Random();
    me = FindFreeEnemy();
    if (me == 0)
    {
    ...
    while (ok)
    {
        ...
        for (j = ENEMYMIN; j <= ENEMYMAX; j++)
        {
            ...
            {
                if (Distance(me, j) < 10)
                    ok = 1; // too close to other enemies
            }
        }
        trys++;
        if (trys == 40)
    ...
    /* Drue - Random() returns a value [0, 255] and 20% is 51
     * To keep the code simple because it was already a mess,
     * I'm wrapping the original statement with my if (Random() > 51)
     * This says, do what was originally here 80% of the time, otherwise do my new item
     */
    if (Random() > 51)
    {
        ...
        if (Random() > 64)
        {
            ...
            {
                CreateSprite(me, big0, big1, 10, big3, 118, initY, 21, 21,
                             -EnemySpeed, 0, Levels[CurrentLevel].EnemyLife);
            }
            else
            {
                CreateSprite(me, bender0, bender1, 10, bender3, 118, initY, 11,
                             10, -EnemySpeed, 0,
                             Levels[CurrentLevel].EnemyLife);
            }
        }
        else
        {
            CreateSprite(me, cute0, cute1, 10, cute3, 118, initY, 11, 11,
                         -CuteSpeed, 0, Levels[CurrentLevel].EnemyLife);
        }
    }
    // gets here 20% of time
    else
    {
        // uses random number generated at time and breaks it down to display each color
        // should be roughly 25%
        if (random_gen_pac_ghost > 191)
            CreateSprite(me, pac_ghost_green, pac_ghost_green, 10, pac_ghost_blank, 118, initY,
                                 18, 24, -50, 0, Levels[CurrentLevel].EnemyLife * 6);
        else if (random_gen_pac_ghost > 127)
            CreateSprite(me, pac_ghost_red, pac_ghost_red, 10, pac_ghost_blank, 118, initY,
                                 18, 24, -50, 0, Levels[CurrentLevel].EnemyLife * 6);
        else if (random_gen_pac_ghost > 64)
            CreateSprite(me, pac_ghost_orange, pac_ghost_orange, 10, pac_ghost_blank, 118, initY,
                                 18, 24, -50, 0, Levels[CurrentLevel].EnemyLife * 6);
        else
            CreateSprite(me, pac_ghost_blue, pac_ghost_blue, 10, pac_ghost_blank, 118, initY,
                                 18, 24, -50, 0, Levels[CurrentLevel].EnemyLife * 6);
...
void MoveEnemiesThread(UArg arg0)
{
    uint32_t enemy;
    uint32_t t, j;
    while (1)
        ...
        for (enemy = ENEMYMIN; enemy <= ENEMYMAX; enemy++)
        {
            if (Things[enemy].life)
            {
                ...
                { // move towards ship
                    ...
                    Things[enemy].vx = dx;
                    Things[enemy].vy = dy
                            + (((FIX / 2) * Random()) / 256 - FIX / 4); // wiggles around
                }
                else
                {
                ...
                if (Random16() < Levels[CurrentLevel].EnemyMissileThreshold)
                { // 0 to 65535
                    j = FindFreeEMissile();
                    if (j)
                    {
                        int MissileSpeed =
                                Levels[CurrentLevel].EnemyMissileSpeed;
                        if (CurrentLevel < 2)
                        {
                            CreateSprite(j, missile0, missile1, 10, missile3,
                                         Things[enemy].x - 2,
                                         Things[enemy].y - 2, 5, 5,
                                         -MissileSpeed, 0, 1);
                        }
                        else
                        {
...
void ButtonTask(void)
{
    ...
    {     // Button1 was pressed
        ...
        if (i)
        {
            ...
            if (Score > 1)
                Score--;
            CreateSprite(i, rocket0, rocket1, 10, rocket3, Things[SHIP].x + 8,
                         Things[SHIP].y - 5, 8, 3, RocketSpeed, 0, 1);
        }
    }
    if (current && (last == 0))
    { // release
...
void SlowPeriodicTask(UArg arg0)
{
    // runs at about 10 Hz
    if (IntermissionFlag)
    {      // halt game during intermissions
        if (Random16() < Levels[CurrentLevel].EnemyThreshold)
        { // 0 to 65535
...
{
    uint16_t x, y;
    uint8_t select; // joystick used to seed random number
...
// SlowPeriodicTask runs at 10 Hz
    ...
    Task_construct(&tsk1Struct, (Task_FuncPtr) MoveEnemiesThread, &tskParams,
                   NULL);
